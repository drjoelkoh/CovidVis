...
# Define phase dates and rectangles for background coloring
phase_rectangles <- tibble(
  ...
  # Choose colors similar to wildfire chart - you can adjust these
...
# Determine y-limit to place labels lower
max_y <- max(covid_long$Rate, na.rm = TRUE)
label_y <- max_y * 0.6  # lower the phase labels to 60% height
...
density_plot <- ggplot(covid_long, aes(x = Date, y = Rate, color = Group)) +
  ...
  geom_rect(data = phase_rectangles, 
   ...
   # Add manual fill scale for the rectangles
  ...
  # Phase labels (rotated and shifted down)
  geom_text(
    data = phase_dates,
...
# Save the plot
ggsave("covid_density_rates_sg_phases_shifted_down.png",
       plot = density_plot,
